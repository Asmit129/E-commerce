...
const Header = () => {
  ...
  const isHeaderPinned = scrolled || isHomeRoute;
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 border-b ${
      isHeaderPinned
        ? `backdrop-blur-xl border-[var(--border-glass)] ${
            scrolled
              ? "bg-[var(--bg-main)]/90 shadow-[0_4px_30px_rgba(0,0,0,0.5)]"
              : "bg-[var(--bg-main)]/78 shadow-[0_4px_24px_rgba(0,0,0,0.18)]"
          }`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 lg:gap-8">
          {[
            ...
            const isActive = link.path === "/" ? isHomeRoute : location.pathname === link.path;
            return (
              <Link key={link.path} to={link.path}
                className={`text-sm font-medium transition-colors hover:text-[var(--accent-gold)] ${isActive ? 'text-[var(--accent-gold)]' : 'text-[var(--text-soft)]'}`}>
                {link.label}
              </Link>
            );
          })}
          ...
          {[
            ...
            return (
              <Link key={link.path} to={link.path} onClick={() => setMobileMenuOpen(false)}
                className={`block text-sm font-medium py-2 ${isActive ? 'text-[var(--accent-gold)]' : 'text-[var(--text-soft)]'}`}>
                {link.label}
              </Link>
            );
          })}
