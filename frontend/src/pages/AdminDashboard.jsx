...
import { ShieldCheck, PackageOpen, Users, LogOut, CheckCircle, XCircle, Clock, Trash2, ArrowRight, Handshake, Headset, MessageSquare, Check, BookOpen, Tag } from "lucide-react";
                     ...
                     {articles.map(art => (
                        <div key={art._id} className="bg-[var(--bg-card)] border border-[var(--border-glass)] rounded-xl overflow-hidden flex flex-col">
                           ...
                           <div className="p-4 flex-1 flex flex-col">
                             ...
                             <Link to={`/journal/${art.slug}`} target="_blank" className="text-xs font-bold uppercase tracking-wider text-[var(--accent-gold)] hover:text-[var(--text-main)] transition-colors">Read Live Mode ↗</Link>
                           </div>
                        </div>
                     ))}
                  </div>
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-[var(--bg-card)] border border-[var(--border-glass)] rounded-2xl shadow-2xl overflow-hidden">
                      <div className="p-6 border-b border-[var(--border-glass)] flex justify-between items-center sticky top-0 bg-[var(--bg-card)] z-10">
                        ...
                        <button onClick={() => setShowArticleModal(false)} className="text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors">
