    ...
    <PageTransition className="bg-[var(--bg-main)] min-h-screen">
      <section className="relative h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 text-center pt-20">
          <div key={currentSlide}>
            ...
            <p className="text-lg md:text-xl text-white/80 drop-shadow-md max-w-2xl mx-auto mb-10 font-light">
              {heroSlides[currentSlide].desc}
            </p>

            <form onSubmit={handleSearch} className="max-w-xl mx-auto relative mb-8 group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Search className="h-5 w-5 text-white/60 group-focus-within:text-[var(--accent-gold)] transition-colors" />
              </div>
              <input
                type="text"
                className="w-full bg-white/10 backdrop-blur-md border border-white/15 rounded-full py-4 pl-12 pr-32 text-white placeholder-white/55 focus:outline-none focus:border-[var(--accent-gold)] focus:ring-1 focus:ring-[var(--accent-gold)] transition-all"
      ...
      <section className="border-y border-[var(--border-glass)] bg-[var(--bg-black)]/40 backdrop-blur-md">
        <div className="wrap py-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center divide-x divide-[var(--border-glass)]">
            ...
            <div key={cat.id}>
              <Link to={cat.link} className="group relative block h-[400px] rounded-2xl overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  ...
                  <p className="text-white/75 drop-shadow-sm text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 line-clamp-2">
                    {cat.description}
                  </p>
      ...
      <section className="section bg-gradient-to-b from-[var(--bg-main)] to-[var(--bg-black)] relative">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {featuredProducts.map((p) => (
                <div key={p._id} className="group h-full flex flex-col">
